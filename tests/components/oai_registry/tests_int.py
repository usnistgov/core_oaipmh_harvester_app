...
class TestUpsertSetForRegistry(IntegrationBaseTestCase):
    ...
    def test_upsert_updates_if_does_exist(self):
        ...
        # Assert
        set_in_database = (
            oai_harvester_set_api.get_by_set_spec_and_registry_id(
...
class TestHandleDeleteSet(IntegrationBaseTestCase):
    ...
    def test_handle_deleted_set_deletes_set(self):
        ...
        # Act
        oai_registry_api._handle_deleted_set(
            self.fixture.registry.id, sets_response
        )
    ...
    for metadata_format in metadata_formats:
        obj_in_database = next(
            (
                x
                for x in mock
                if x.metadata_prefix == metadata_format.metadata_prefix
            ),
            [None],
        )
        if obj_in_database != [None]:
