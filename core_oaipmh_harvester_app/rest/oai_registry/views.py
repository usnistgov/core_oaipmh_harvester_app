    ...
    def post(self, request):
        ...
        try:
            ...
            content = OaiPmhMessage.\
                get_message_labelled('Registry {0} added with success.'.format(registry.name))
...
class RegistryDetail(APIView):
    ...
    def patch(self, request, registry_id):
        ...
        try:
            registry = oai_registry_api.get_by_id(registry_id)
            # Build serializer
            serializer = serializers.UpdateRegistrySerializer(instance=registry,
                                                              data=request.data)
            # Validate data
            serializer.is_valid(True)
            # Save data
...
class Harvest(APIView):
    ...
    def put(self, request, registry_id):
        ...
        try:
            ...
            registry_metadata_formats = oai_metadata_format_api.\
                get_all_by_registry_id(registry_id).values_list('id')
            ...
            oai_metadata_format_api.\
                update_for_all_harvest_by_list_ids(registry_metadata_formats, False)
            ...
            content = OaiPmhMessage.\
                get_message_labelled('Registry harvesting configuration updated with success.')
